...
function log(message) {
    ...
    try { print(text); } catch (error) { java.lang.System.out.println(text); }
}
...
var PROFILES = {
    ...
    earth8000: {name:"earth8000", sourceScale:10, resize:50, effectiveScale:8000, width:5376, height:2688},
    earth4000: {name:"earth4000", sourceScale:10, resize:100, effectiveScale:4000, width:10752, height:5376},
    earth2000: {name:"earth2000", sourceScale:20, resize:100, effectiveScale:2000, width:21504, height:10752},
    production: {name:"production", sourceScale:40, resize:100, effectiveScale:1000, width:43008, height:21504}
};
var PROFILE_ALIASES = {preview:"earth4000"};
var SOURCE_DIMENSIONS = {10:[10752,5376], 20:[21504,10752], 40:[43008,21504]};
var selectedProfileName, runPreflightOnly;
...
function resolveBuiltInLayer(name) {
    ...
    log("Resolved built-in layer: "+name);
    return layer;
}
...
function validateVersion(versionText) {
    ...
    if (!(v[0] === 2 && v[1] === 27 && v[2] === 0)) log("WARNING: WorldPainter "+versionText+" is newer or otherwise untested; 2.27.0 is verified.");
}
function requiredAssetNames(config) {
    var suffix=String(config.sourceScale)+"k.png";
    var assets=["images/HeightMap"+suffix,"images/BiomeMap"+suffix,"images/WaterMap"+suffix,"images/Ice"+suffix];
    if (config.sourceScale === 40) return assets.concat(["images/globecover1_40k.png","images/globecover2a_40k.png","images/globecover2b_40k.png","images/globecover3_40k.png","images/globecover4_40k.png"]);
    return assets.concat(["images/globecover"+suffix]);
}
function validateConfiguration(config) {
    if (!config) fail("unknown profile '"+selectedProfileName+"'; supported profiles: smoke, preview, earth8000, earth4000, earth2000, production");
    ...
    var base=SOURCE_DIMENSIONS[config.sourceScale];
...
function runXenoEarth(profileName, preflightMode) {
    selectedProfileName=String(profileName || "smoke");
    selectedProfileName=PROFILE_ALIASES[selectedProfileName] || selectedProfileName;
    runPreflightOnly=Boolean(preflightMode);
    log("[1/9] Preflight");
    config=PROFILES[selectedProfileName]; validateConfiguration(config); scale=config.sourceScale; resize=config.resize;
    var minimumX=-config.width/2, maximumX=config.width/2-1, minimumZ=-config.height/2, maximumZ=config.height/2-1;
    var horizontalScaleFactor = config.sourceScale * config.resize / 100.0;
    var spawnX = Math.round(110.5 * horizontalScaleFactor);
    var spawnZ = -Math.round(11.4 * horizontalScaleFactor);
    if (spawnX < minimumX || spawnX > maximumX || spawnZ < minimumZ || spawnZ > maximumZ) fail("spawn point is outside generated profile bounds");
    ...
    log("WorldPainter version: "+wpVersion+"\nSelected profile: "+config.name+"\nSource scale: "+scale+"k\nResize percentage: "+resize+"%\nEffective scale: 1:"+config.effectiveScale+"\nOutput dimensions: "+config.width+" x "+config.height+"\nExpected bounds: "+minimumX+".."+maximumX+", "+minimumZ+".."+maximumZ+"\nSpawn coordinates: "+spawnX+", "+spawnZ+"\nRequired source assets: "+requiredAssetNames(config).join(", ")+"\nMap-format ID: "+LEGACY_ANVIL_MAP_FORMAT+"\nBuild limits: "+LOWER_BUILD_LIMIT+".."+(UPPER_BUILD_LIMIT-1)+"\nWater level: "+seaLevel);
...
heightMap = null;
    ...
    log("[4/9] Applying surface terrain");
    ...
    var manifest={formatVersion:1,profile:config.name,targetMinecraftVersion:targetMinecraftVersion,sourceScale:scale,resize:resize,effectiveScale:config.effectiveScale,scale:config.effectiveScale,width:config.width,height:config.height,minimumX:minimumX,maximumX:maximumX,minimumZ:minimumZ,maximumZ:maximumZ,minimumSurfaceY:minimumSurfaceY,maximumSurfaceY:maximumSurfaceY,seaLevel:seaLevel,projection:"equirectangular",populationMode:"pregenerated",caves:false,ores:false,lava:false,structures:false,vegetation:true};
    ...
    log("*** REQUIRED MANUAL EXPORT CHECKLIST: disable Populate, Resources, Caves, Caverns, Chasms, Ravines, Structures, lava, and all unlisted generators; keep bottomless world OFF. ***");
